...
class TypeCheckVisitor(GenericVisitor): 
    ...
    def visit_LetStmt(self, node):
        ...
        if simbolo is None: 
            ...
            tipoExpressao = self.visit(node.exp)
            if tipoVariavel != tipoExpressao: 
                self.erro("Variável não compatível com expressão")
    ...
    def visit_SumExpr(self, node):
        tipoEsquerda = self.visit(node.left)
        tipoDireita = self.visit(node.right)
        if tipoEsquerda == self.INT() and tipoDireita == self.INT(): 
            return self.INT()
        
    def visit_IfStmt(self, node):
        tipoCondicao = self.visit(node.cond)
        if tipoCondicao == self.BOOLEAN():
            for stm in node.body: 
                self.visit(stm)
        else: 
            self.erro("Condição do IF deve ser booleana")
